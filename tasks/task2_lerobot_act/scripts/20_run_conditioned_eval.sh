...
if [ "$#" -lt 3 ] || [ "$#" -gt 4 ]; then
  echo "Usage: bash scripts/20_run_conditioned_eval.sh <condition_type> <model_run_id> <checkpoint> [full|debug-only]" >&2
  exit 2
fi
...
chain_id="${model_run_id}_${condition_type}_eval_chain"
...
trap 'rc=$?; if [ "$rc" -ne 0 ]; then write_status failed "$rc" "conditioned eval chain failed; inspect log_file"; fi' EXIT
. .venv/bin/activate
...
echo "[$(timestamp)] Conditioned eval chain started: model=${model_run_id}, condition=${condition_type}, mode=${mode}"
...
echo "[$(timestamp)] Conditioned eval chain finished: model=${model_run_id}"
