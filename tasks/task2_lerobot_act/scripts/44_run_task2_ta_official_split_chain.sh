...
trap 'rc=$?; if [ "$rc" -ne 0 ]; then write_status failed "$rc" "TA official split chain failed; inspect log_file"; fi' EXIT
...
lines = [
    "# HW3 Task 2 TA Official Split Audit",
    "",
...
download_ta_splits() {
  write_status running null "downloading TA official split dataset"
...
PY
  echo "[$(timestamp)] Patch TA episodes_stats count fields for LeRobot v3 converter"
  python scripts/47_patch_ta_stats_counts.py \
    --root="${source_root}" \
    --summary="results/task2_ta_stats_count_patch.json"
  write_audit
}
...
root = path.parents[1]
required = [root / "meta" / "stats.json"]
if path.exists() and json.loads(path.read_text()).get("codebase_version") == "v3.0" and all(p.exists() for p in required):
    sys.exit(0)
sys.exit(1)
PY
  then
  ...
  rm -rf "$target_dir"
  ln -sfnT "$(pwd)/${source_dir}" "${v30_root}/xiaoma26/${repo_leaf}"
...
train_model() {
  ...
  export HW3_TA_CHUNK_SIZE="$chunk_size"
...
eval_model() {
  ...
  local task_meta_split="$split"
  if [ "$task_meta_split" = "ABC" ]; then
    task_meta_split="A"
  fi
  HW3_TA_TASK_META="${v30_root}/xiaoma26/calvin_lerobot_split${task_meta_split}/meta/tasks.parquet" \
    bash scripts/45_run_ta_conditioned_eval.sh "$condition" "$eval_run" "$checkpoint" full
}

summarize_ta() {
  write_status running null "summarizing TA official split results"
  python - <<'PY'
from pathlib import Path
import csv
import json
...
payload = {
    "source": "TA official split dataset xiaoma26/calvin-lerobot",
...
echo "[$(timestamp)] TA official split chain started."
...
echo "[$(timestamp)] Patch converted TA image stats containers"
python scripts/48_patch_ta_v30_image_stats.py \
  --root="${v30_root}/xiaoma26" \
  --summary="results/task2_ta_v30_image_stats_patch.json"
write_audit

write_status running null "TA official smoke training"
train_model A onehot act_ta_a_taskcond_smoke_seed0 200 50

train_model A onehot act_ta_a_taskcond_100k_seed0 100000 50
eval_model onehot act_ta_a_taskcond_100k_seed0 50000 act_ta_a_taskcond_100k_seed0_ckpt050k A
eval_model onehot act_ta_a_taskcond_100k_seed0 100000 act_ta_a_taskcond_100k_seed0 A
summarize_ta || true

train_model B onehot act_ta_b_taskcond_100k_seed0 100000 50
eval_model onehot act_ta_b_taskcond_100k_seed0 50000 act_ta_b_taskcond_100k_seed0_ckpt050k B
eval_model onehot act_ta_b_taskcond_100k_seed0 100000 act_ta_b_taskcond_100k_seed0 B
summarize_ta || true

train_model C onehot act_ta_c_taskcond_100k_seed0 100000 50
eval_model onehot act_ta_c_taskcond_100k_seed0 50000 act_ta_c_taskcond_100k_seed0_ckpt050k C
eval_model onehot act_ta_c_taskcond_100k_seed0 100000 act_ta_c_taskcond_100k_seed0 C
summarize_ta || true

train_model ABC onehot act_ta_abc_taskcond_100k_seed0 100000 50
eval_model onehot act_ta_abc_taskcond_100k_seed0 50000 act_ta_abc_taskcond_100k_seed0_ckpt050k ABC
eval_model onehot act_ta_abc_taskcond_100k_seed0 100000 act_ta_abc_taskcond_100k_seed0 ABC
summarize_ta || true

train_model ABC sbert act_ta_abc_lang_sbert_100k_seed0 100000 50
eval_model sbert act_ta_abc_lang_sbert_100k_seed0 50000 act_ta_abc_lang_sbert_100k_seed0_ckpt050k ABC
eval_model sbert act_ta_abc_lang_sbert_100k_seed0 100000 act_ta_abc_lang_sbert_100k_seed0 ABC
summarize_ta || true

train_model ABC onehot act_ta_abc_taskcond_chunk100_100k_seed0 100000 100
eval_model onehot act_ta_abc_taskcond_chunk100_100k_seed0 100000 act_ta_abc_taskcond_chunk100_100k_seed0 ABC
summarize_ta || true

bash scripts/08_download_important_results.sh > results/task2_ta_official_split_latest_package_path.txt || true
write_status finished 0 "TA official split chain completed"
echo "[$(timestamp)] TA official split chain finished"
