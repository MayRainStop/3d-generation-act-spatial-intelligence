...
if [ -f "$ta_status" ] && [ "$(json_field "$ta_status" status)" = "running" ]; then
  write_status finished 0 "course split chain already running"
  echo "[$(timestamp)] course split chain is already running; queue exits."
  exit 0
fi

write_status running null "starting course split chain"
...
write_status finished 0 "course split chain finished"
